...
|---|---|---|---|---|
| read | `path`,`offset?`,`limit?` | 文件内容分块（行级分页） | 路径在工作区内且可读；支持通过 offset/limit 对大文件进行多次分块读取 | 路径越界、文件不存在、读取失败；offset/limit 非法被归一化为默认值 |
...
## 3. 大文件读取策略（read）
- `read` 工具按**行级分页**返回内容，不再推荐一次性读取整文件：
  - `offset`：可选，默认 `1`，表示从第几行开始（1 基）。  
  - `limit`：可选，默认 `50`，表示最多返回多少行内容。  
  - 当未显式提供 `limit` 或提供非正数时，统一视为 `50`；当 `limit` 过大时，工具内部会按上限裁剪。
- 当 `offset` 大于文件总行数时：
  - 视为到达 EOF，返回 `ok=true` 且 `content=""`，用于明确“没有更多内容可读”的语义。
- 使用建议：
  - 只需查看 README 末尾配置段时，建议优先通过 `offset/limit` 仅读取尾部若干行，而不是读取整份文档。
