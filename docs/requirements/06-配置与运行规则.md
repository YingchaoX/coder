...
- `skills`：技能扫描路径；预装技能（如 create-skill）内嵌在二进制中，不占 paths，用户路径可覆盖同名预装。
- `storage`：SQLite 目录等。

界面约束：
- 不提供主题或字体配置，使用**终端默认样式**（等宽字体）。

命令相关运行规则：
- `/new`、`/resume`、`/compact`、`/diff`、`/undo` 以及 `/mode`（或 `/plan`、`/default`、`/auto-edit`、`/yolo`）、`/tools`、`/skills`、`/todos` 为首发内建命令集合的一部分。
