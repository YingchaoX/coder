...
## 10. `fetch` 工具
### `fetch`
- 输入：`url,method?,headers?,body?,timeout_sec?,max_size_kb?,auth?`
- 输出：`{ok,status_code,content_type,is_image,content,size_bytes,error?}`
- 行为：
  - 发起HTTP请求获取远程资源
  - 自动检测内容类型：文本类（HTML/JSON/TEXT等）或图片类（JPEG/PNG/GIF等）
  - 文本内容自动截断至100KB（由配置决定）
  - 图片内容自动转base64编码（最大1MB，由配置决定）
  - 401错误时自动重试带认证信息
  - 支持Basic/Bearer Token/Cookie等多种认证方式
- 关键约束：
  - 路径必须为合法HTTP/HTTPS URL
  - 响应大小受配置限制以防止过大资源
  - 遵循TLS证书验证策略（可配置跳过内网自签证书）
...
## 8. `question` 工具
- 输入：`questions[]`，每项含 `question`(string)、`options[]{label,description}`
- 输出：格式化文本 `User has answered your questions: "Q1"="answer1", "Q2"="answer2". ...`
- 行为：
  - 仅 plan mode 可用（`defaultToolSet` 中默认 `false`，plan profile 显式 `true`）。
  - 通过 `context.Value` 获取 `QuestionPrompter` 接口（由 REPL `runtimeController` 实现）。
  - 模型一次可发送多个问题，终端逐个展示、逐个收集回答。
  - 每个问题渲染为编号列表，第一个选项自动追加 `(Recommended)`。
  - 用户输入数字 → 返回对应 option 的 `label`；输入文本 → 原样返回作为自定义回复。
  - Esc 取消全部问题，返回 `"User dismissed the questions."` 给模型。
  - 非 TTY 环境（无 `QuestionPrompter`）返回 fallback 提示，模型自行判断。
- 关键类型：
  - `QuestionOption{Label, Description}`
  - `QuestionInfo{Question, Options}`
  - `QuestionRequest{Questions}`
  - `QuestionResponse{Answers, Cancelled}`
  - `QuestionPrompter` 接口：`PromptQuestion(ctx, QuestionRequest) (*QuestionResponse, error)`
- context 注入：`WithQuestionPrompter(ctx, p)` / `QuestionPrompterFromContext(ctx)`（与 `ApprovalPrompter` 对称）
